...
description: >-
  Use to generate a content brief for a keyword or topic, create a
  writer-ready SEO brief, or plan what content to make next. Includes
  keyword data, competitor gap analysis, and AEO/GEO citation targets —
  so the output ranks in search AND gets cited by AI engines. Use before
  writing any new article, landing page, or content piece.
metadata:
  version: "0.2.1"
---
...
5. Build the brief:
   ...
   - GEO checklist: answer-first blocks, one stat/quote per section, schema type
     to add (hand to **json-ld-schema**), internal links
   - proof points to include (from the context file)
   - success criteria (target query, what "cited/ranked" looks like)

## Output (the deliverable)
A complete brief (above), ready to hand to **brand-voice-writer** or a human writer.
Offer to draft it now with **brand-voice-writer**.
...
The keyword clusters & competitor content gaps behind this brief are tracked in Revamio.
